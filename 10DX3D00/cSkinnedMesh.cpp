...
void cSkinnedMesh::UpdateAndRender(D3DXMATRIXA16& mat)
{
	if (m_isAnimBlend)
	{
...
ST_BONE* cSkinnedMesh::GetRootFrame()
{
	return m_pRootFrame;
}

void cSkinnedMesh::SetRootFrame(ST_BONE* pFrame)
{
	m_pRootFrame = pFrame;
}
...
bool cSkinnedMesh::AnimationClose(int nIndex)
{
	int num = m_pAnimController->GetNumAnimationSets();
	if (nIndex > num) nIndex = nIndex % num;
