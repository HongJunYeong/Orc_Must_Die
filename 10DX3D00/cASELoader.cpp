...
void cASELoader::ProcessMESH(OUT cFrame * pFrame)
{
	...
	for (size_t i = 0; i < vecVertex.size(); i++)
	{
		...
		D3DXVec3TransformCoord(&vecVertex[i].n, &vecVertex[i].n, &matInvWorld);
	}
...
void cASELoader::ProcessMESH_TVERT_LIST(OUT vector<D3DXVECTOR2>& vecVT)
{
	int nLevel = 0;
...
void cASELoader::ProcessMESH_TFACE_LIST(OUT vector<ST_PNT_VERTEX>& vecVertex, IN vector<D3DXVECTOR2>& vecVT)
{
	int nLevel = 0;
