...
void cObjLoader::LoadSurface(OUT vector<D3DXVECTOR3>& vecSurface, IN char * szFolder, IN char * szFile, D3DXMATRIXA16 * pMat)
{
	vector<D3DXVECTOR3> vecV;
...
LPD3DXMESH cObjLoader::LoadMesh(OUT vector<cMtlTex*>& vecMtlTex, IN char * szFolder, IN char * szFile)
{
	...
	while (fp != NULL)
	{
		...
		else if (szTemp[0] == 'g')
		{
			/*if (!vecVertex.empty())
			{
				mapVertex[nNumSubset] = vecVertex;
				nNumSubset++;
	...
	for each(auto iter in m_mapMtlTex)
	{
		vecMtlTex[iter.second->GetAttrID()] = iter.second;
	}
