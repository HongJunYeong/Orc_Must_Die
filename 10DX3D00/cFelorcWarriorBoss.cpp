...
void cFelorcWarriorBoss::Update()
{
	...
	//IDLE 상태 3가지 애니메이션 무작위 선택 [다음 애니메이션]
	int n2 = rand() % 3 + 1;
	int nAniTemp;
...
void cFelorcWarriorBoss::Render()
{
	if (!m_isDie)
	{
		...
		RWeaponSphere_Render("felorcwarriorboss_Bone110"); //오른쪽 무기 범위 표시
		LWeaponSphere_Render("felorcwarriorboss_Bone103");//왼쪽 무기 범위 표시
	}
}
